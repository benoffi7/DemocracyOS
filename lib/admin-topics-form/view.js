...
import labelTemplate from './label.jade';
...
export default class TopicForm extends FormView {
  ...
  constructor(topic, forum, tags, projectstypes, users, labels) {
    super();
    this.setLocals(topic, forum, tags, projectstypes, users, labels);
  ...
  setLocals(topic, forum, tags, projectstypes, users, labels) {
    if (topic) {
    ...
    }else if(this.topic.author.id == user.id){
	this.showSelectUsers= false;
	this.actualAuthor= this.topic.author.id;
    }else{
	this.actualAuthor= this.topic.author.id;
    }
    ...
    this.locals = {
      ...
      actualAuthor: this.actualAuthor,
      tags: this.tags,
      projectstypes: this.projectstypes,
      labels: this.labels,
      users: this.users,
      moment: moment,
      forum
  ...
  switchOn() {
    this.bind('click', '[data-hide-voted]', this.bound('onHideVotedClick'));
    this.bind('click', 'input.imauthor', this.bound('onauthorclick'));

    this.bind('click', 'a.add-link', this.bound('onaddlinkclick'));
    this.bind('click', 'a.remove-link', this.bound('onremovelinkclick'));
    this.bind('click', 'a.add-label', this.bound('onaddlabelkclick'));
    this.bind('click', 'a.remove-label', this.bound('onremovelabelkclick'));
 ...
 onHideVotedClick (ev) {
    if (ev.delegateTarget.checked){
	this.find('#select-users').addClass("hide");
    }else{
	this.find('#select-users').removeClass("hide");
    }
  }
  ...
  onaddlabelkclick (ev) {
    ...
    ev.preventDefault();
    let labelId= this.find('form #labels-select')[0].value;  

    if(this.topic.labels.length <= 6){
        topicStore.addLabel(id, labelId).then(() => {
            let labelName= "";
            ...
            console.log(this.topic.labels);
            let add= true;
            this.topic.labels.forEach(label => {
                if(label.id == labelId){
                    add= false;                
                }
            });        
            if(add){
                var labels = o('.topic-labels-des', this.el);
                var label = render(labelTemplate, {
                    label: {id: labelId, name: labelName}
                });
                labels.append(o(label));
                this.topic.labels.push({id: labelId, name: labelName});
            }
        }).catch(err => {
            console.log(err);
        ...
        alert("Maximo 7 Etiquetas");
    }
  }
  
  onremovelabelkclick (ev) {
    let value;
    let id = this.topic.id;
    ...
    let labelId= el.getAttribute('label-id');
  
    topicStore.deleteLabel(id, labelId).then(() => {
        el.parentElement.remove();
        this.topic.labels.forEach(function(label, index, object) {
            if (label.id === labelId) {
                object.splice(index, 1);
            }
        });
//        for (var i = 0; i < this.topic.labels.length; i++) {
//            console.log(this.topic.labels[i]);
//        } 
    }).catch(err => {
        console.log(err);
        log('Failed cast %s for %s with error: %j', value, id, err);
  ...
  renderToggles() {
	...
	toggle2.label('Yes', 'No');
    	toggle2.name('imauthor');
    	...
    	this.find('.imauthor-toggle').append(toggle2.el);
    }*/
  }
