...
import { findAllUsersTypes, findAllActiveUsersTypes, findUserType, clearUsersTypesStore } from '../usertype-middlewares/usertype-middlewares';
import { findAllUserList, findAllFilterUserList, findUser, clearUserListStore } from '../userlist-middlewares/userlist-middlewares';
import { findAllProjectsTypes, findAllActiveProjectsTypes, findProjectType, clearProjectsTypesStore } from '../projecttype-middlewares/projecttype-middlewares';
...
page(forumRouter('/admin/topics/create'), clearTagStore, findAllActiveTags,
                                        clearProjectsTypesStore, findAllActiveProjectsTypes,
					findAllUserList, ctx => {
  ctx.sidebar.set('topics');
...
page(forumRouter('/admin/topics/:id'), clearTagStore, findAllActiveTags,
  ...
  let form = new TopicForm(ctx.topic, ctx.forum, ctx.tags, ctx.projectstypes, ctx.userlist);
  form.replace('.admin-content');
...
page(forumRouter('/admin/projectstypes'), clearProjectsTypesStore, findAllProjectsTypes, ctx => {
  let currentPath = ctx.path;
...
function valid(ctx, next) {
  ...
  if (/topics|tags|users|userlist|userstypes|projectstypes/.test(section)) return next();
  if (/topics|tags|users|userlist|userstypes|projectstypes\/create/.test(section)) return next();
  if (/topics|tags|users|userlist|userstypes|projectstypes\/[a-z0-9]{24}\/?$/.test(section)) return next();
}
