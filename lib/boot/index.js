...
/**
 * ConfigHome API Service
 */

app.use('/api', require('lib/confighome-api'));
...
app.use('/api', require('lib/projecttype-api'));

/**
 * Label API Service
 */

app.use('/api', require('lib/label-api'));

/**
 * UserType API Service
 */

app.use('/api', require('lib/usertype-api'));
...
/**
 * Root API Image Service
 */

app.use('/api-image', require('lib/api-image'));

/**
 * UserType API IMAGE Service
 */

app.use('/api-image', require('lib/usertype-api-image'));

/**
 * ConfigHome API IMAGE Service
 */

app.use('/api-image', require('lib/confighome-api-image'));

/**
 * GET Uploads Image
 */

app.use('/uploads', require('lib/uploads'));
...
//Esto esta aca porque antes si no como que no carga los modelos
var api = require('lib/db-api');

/*
 * Se modifica la configuracion por defecto en base a la configuracion de la base de datos
 */
app.use(function (req, res, next) {
  api.confighome.get(config.configId, function (err, confighomeDoc) {
    if (err){
        config.text= "Bienvenido";
        next();
    }
    config.headerBackgroundColor= confighomeDoc.menuColor;
    config.logo= confighomeDoc.logo;
    config.text= confighomeDoc.text;
    log('Configuracion Cambiada del lado del Servidor');
...
app.use(require('lib/infohome'));
app.use(require('lib/profile'));
app.use(require('lib/favorites'));
app.use(require('lib/confighome'));
