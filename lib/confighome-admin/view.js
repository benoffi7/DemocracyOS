...
export default class ConfigHome extends FormView {
  ...
  postserialize(formData, data) {
    formData = formData || {};
    data = data || {};

    formData.append('acercade', JSON.stringify(serializer.toArray(data.body)));
    //formData.delete('body');
    formData.append('comofunciona', JSON.stringify(serializer.toArray(data.bodyComo)));
    //formData.delete('bodyComo');
