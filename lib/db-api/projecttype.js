...
exports.allActiveOfUsertype = function allActiveOfUsertype(projects, fn) {
  log('Looking for all projectstypes active of usertype.');

  var ids = [];
  if(projects){
    ids= projects.map(function(id) { return mongoose.Types.ObjectId(id); });
  }
  var query = {deletedAt: null, _id: {$in: ids}};
