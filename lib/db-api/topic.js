...
var projecttypeApi = require('./projecttype');
...
exports.all = function all(params, fn) {
  ...
  Topic
  .find(query)
  .select('id hash topicId mediaTitle tag projecttype participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author forum clauses')
  .populate('tag', 'id hash name color image')
  .populate('projecttype', 'id hash name')
  .populate('author', 'id firstName lastName username fullName email profilePictureUrl avatar usertype')
  .sort({ publishedAt : -1 })
...
exports.allHome = function allHome(params, fn) {
  log('Looking for all topics home.');

  var query = { deletedAt: null, publishedAt: { $ne: null}};

  if (params.forum) query.forum = params.forum;
  
  if (params.search)findSearch= query.mediaTitle= {'$regex': params.search, $options:"i"};
  log(query);
  findTag= '';
  if (params.tag && params.tag != -1)findTag= {'tag': params.tag};
  findProject= '';
  if (params.project && params.project != -1) findProject={'projecttype': params.project};
  fundUser= '';
  if (params.user && params.user != -1) fundUser={'author': params.user};
  findState= '';
  if(params.state){
      if(params.state == '-1') findState={'closingAt': { $ne: null}};
      if(params.state == '1') findState={'closingAt': null};
  }

  var sort= {publishedAt : -1};
  if(params.sort && params.sortValue){
      if(params.sort == 'date'){
          sort= {publishedAt : params.sortValue};
      }else{
          //Esto no funciona
          sort= {participants : params.sortValue};
      }   
  }
...
exports.allUser = function allUser(params, fn) {
  log('Looking for all topics user.');
...
exports.favorites = function favorites(params, fn) {
  log('Looking for all topics favorites.');
  
  var ids = params.favorites.map(function(id) { return mongoose.Types.ObjectId(id); });
...
exports.allByTag = function allByTag(tagId, fn) {
  log('Looking for all topics by tag ' + tagId);
  ...
  .exec(function (err, topics) {
    ...
    log('Delivering all topics %j', pluck(topics, 'id'));
    fn(null, topics);
  });
...
/**
 * Get all Topics by Project Type
 *
 * @param {Function} fn callback function
 *   - 'err' error found while process or `null`
 *   - 'users' list items found or `undefined`
...
exports.allByProjectType = function allByProjectType(projectId, fn) {
  log('Looking for all topics by projecttype ' + projectId);
  
  Topic
  .find({projecttype: projectId})
  .exec(function (err, topics) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
...
exports.create = function create(data, fn) {
  ...
  ensureTag(data, function(err, topic) {
    ...
    ensureProjectType(topic, function(err, topic){
        if (err) {
            log('Found error from topic creation: %s', err.message);
            return fn(err);
        }
...
function ensureTag(data, fn) {
  tagApi.searchOne(data.tag, function(err, tags) {
    if (err) {
...
function ensureProjectType(data, fn) {
  projecttypeApi.searchOne(data.projecttype, function (err, projecttype) {
    if (err) return log('Found error %s', err.message), fn(err);

    // now set `data.projecttype` to `projecttype`'s document _id
...
exports.get = function get(id, fn) {
  ...
  .populate('projecttype', 'id hash name')
  .populate('author', 'id firstName lastName username fullName email profilePictureUrl avatar usertype')
  .populate('author.usertype', 'id hash name image')
  .populate('labels', 'id hash name')
  //.populate('participants', 'id firstName lastName username fullName displayName email profilePictureUrl avatar')
  .exec(onget(fn));
};

exports.getHash = function getHash(hash, fn) {
  var query = { hash: hash, deletedAt: null };

  log('Looking for topic %s', hash);
  Topic
  .findOne(query)
  .populate('tag')
  .populate('projecttype', 'id hash name')
...
exports.getWithForum = function getWithForum(id, fn) {
  ...
  Topic
  .findOne(query)
  .populate('tag forum projecttype')
  .populate('author', 'id firstName lastName fullName email profilePictureUrl avatar usertype')
  .exec(onget(fn));
};
...
exports.vote = function vote(id, user, value, fn) {
  ...
  .populate('tag', 'id hash name color image')
  .populate('projecttype', 'id hash name')
  .populate('author', 'id firstName lastName fullName email avatar usertype')
  .exec(function (err, topic) {
    if (err) {
      log('Found error %s', err);
      return fn(err);
