...
var usertypeApi = require('./usertype');
...
exports.allFilter = function allFilter(level, state, fn) {
  log('Looking for all users.' + level + "-" + state);

  var findDelete= "";
  if(state == "0"){//Inactivo
      findDelete= {deletedAt: { $ne: null}};
  }else if(state == "1"){//Activo
      findDelete= {deletedAt: null};
  }
  var findLevel= "";
  if(level != "-1"){//Si es distinto de 1 filtro por level
      findLevel= {level: level};
  }
...
exports.allByUserType = function allByUserType(usertypeId, fn) {
  log('Looking for all users by usertype ' + usertypeId);
  
  User
  .find({usertype: usertypeId})
  .exec(function (err, users) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
...
/**
 * Add Favorite to User for `id` string or `ObjectId`
 *
...
//exports.addFavorite = function addFavorite(id, topicId, fn) {
...
/**
 * Update user by `id` and `data`
 *
 * @param {ObjectId|String} data to create user
 * @param {Function} fn callback function
 *   - 'err' error found on query or `null`
 *   - 'user' item created or `undefined`
 * @return {Module} `user` module
 * @api public
 */

exports.update = function update(id, data, fn) {
  log('Updating user %s with %j', id, data);

  log('Looking for usertype %s in database.', data.usertype);
  usertypeApi.searchOne(data.usertype, function (err, usertype) {
    if (err) return log('Found error %s', err.message), fn(err);

	  // now set `data.usertype` to `usertype`'s document _id
	  ...
	  function onget(err, user) {
		...
		// update and save user document with data
		user.set(data);
		user.save(onupdate);
	  }

	  function onupdate(err, user) {
		if (!err) return log('Saved user %s', user.id), fn(null, user);
...
exports.getWithFav = function getWithFav(id, fn) {
  log('Looking for User %s', id);
...
exports.getProfile = function getProfile(username, fn) {
  log('Looking for User Profile %s', username);
...
exports.lookup = function lookup(ids, fn) {
  ...
  .select('firstName lastName fullName username email avatar profilePictureUrl notifications')
  .exec(function(err, users) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName displayName email avatar usertype staff notifications locale web facebook twitter linkedin googlePlus youtube favorites')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName displayName username avatar notifications locale')(user);
}
