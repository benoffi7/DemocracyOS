...
import autosubmitImage from '../autosubmit/autosubmit-image.js';
...
export default class FormView extends View {
  ...
  constructor (template, options = {}) {
    ...
    this.autosubmitImage('form[autosubmit-image]');
  ...
  oninsert () {
	...
	this.on('responseImage', this.bound('unloading'));
    this.on('responseImage', this.bound('responseImage'));
  }
...
/*
  ...
  autosubmitImage (selector) {
    ...
    let submit = () => {
      self.emit('request');
      let postserialize = self.postserialize ? self.postserialize.bind(self) : null;
      let formRequest = autosubmitImage(el, ((err, res) => self.emit('responseImage', err, res)), postserialize);
      //formRequest.on('abort', self.onCancel.bind(self));
    };
