...
//  , authorUrl: { type: String, validate: validators.isURL({ skipEmpty: true }) }
  , projecttype: { type: ObjectId, ref: 'ProjectType'}
...
TopicSchema.index({ projecttype: -1 });
...
TopicSchema.methods.addLabel = function(labelId, cb) {
  this.labels.addToSet(labelId);
  if (cb) this.save(cb);
};
...
TopicSchema.methods.deleteLabel = function(labelId, cb) {
