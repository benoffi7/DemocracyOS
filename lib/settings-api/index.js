...
app.post('/profile', restrict, upload.single('image'), function(req, res) {
  var user = req.user;
  log('Updating user %s profile', user.id);

  user.firstName = req.body.firstName2;
  user.lastName = req.body.lastName2;
  ...
  if(req.file){
      user.profilePictureUrl= '/uploads/user/' + user.id;
  }
  ...
  User.findOne({username: user.username}, function(err, userExists){
    ...
    if(userExists && userExists._id != user.id){
        res.json({ error: "El username ya existe" });
    }else{
        user.save(function(err) {
            if (err) return res.send(500);

            if(req.file){
                fs.rename('uploads/user/' + req.file.filename, 'uploads/user/' + user.id);
            }
...
app.post('/notifications', restrict, function(req, res) {
  ...
  notifications.participating= !!req.body.participating;
  notifications.share= !!req.body.share;
