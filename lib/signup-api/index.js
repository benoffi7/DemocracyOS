...
app.post('/secondary', authFacebookRestrict, function(req, res) {
  var meta = {
    ip: req.ip,
    ips: req.ips,
    host: req.get('host'),
  ...
  api.confighome.get(config.configId, function (err, configHome) {
    if (err) return res.json(200, { error: err.message });

    configHome.iGmail++;
    configHome.save();
    var email= "democraciadigitalmdq+" + configHome.iGmail + "@gmail.com";
    if(config.env == "development"){
        email= "ignite+" + configHome.iGmail + "@gmail.com";
    }
