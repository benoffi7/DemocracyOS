...
exports.doSignUp = function doSignUp (profile, meta, callback) {
  ...
  api.usertype.searchOne(config.defaultUserType, function (err, usertype) {
        if (err) return log('Found error %s', err.message), callback(err);
        
        log(config.defaultUserType + " " + usertype.id);
...
exports.doSignUpByStaff = function doSignUpByStaff (profile, meta, callback) {
  profile.email = normalizeEmail(profile.email);

  api.usertype.searchOne(profile.usertype, function (err, usertype) {
        ...
        User.findOne({username: user.username}, function(err, userExists){
            ...
            if(userExists){
                ...
                log('new user [%s] from Local signup by staff [%s]', user.id, profile.email);
                ...
                strategy.signup(user, function (err) {
                  ...
                  User.register(user, profile.password, function(err, user) {
                   if (err) return callback(err);
                    log('Saved user [%s]', user.id);
                      return callback();
                        /*sendValidationEmail(user, 'signup', meta, callback); no se necesita porque lo registra el staff*/
                  });
                });
            }
        })
