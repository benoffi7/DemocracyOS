...
export function findAllActiveTags(ctx, next) {
  tagStore.findAllActive().then(tags => {
    ctx.tags = tags;
    next();
  }).catch(err => {
    if (404 !== err.status) throw err;
    log(`Unable to load tags`);
  });
}
...
export function findTagHome(ctx, next) {
  tagStore
    .findOneHash(ctx.params.hashTag)
    .then(tag => {
      ctx.tag = tag;
      return next();
    })
