...
function (req, res) {
  ...
  api.topic.allSearch(params, function(err, topics) {
    if (err) return _handleError(err, req, res);
...
app.get('/topic/newtopics', function (req, res) {
  log('Request /topic/newtopics');
  var select= req.query.select || 0;
  var params= {select: select};
  api.topic.allNew(params, function (err, data) {
    if (err) return _handleError(err, req, res);

    log('Serving all data topics');
    res.json(data);
  });
});

app.get('/topic/allvotes', function (req, res) {
  log('Request /topic/newtopics');
  var select= req.query.select || 0;
  var params= {select: select};
  api.topic.allVotes(params, function (err, data) {
    if (err) return _handleError(err, req, res);
...
app.get('/topic/:id/votes', function (req, res) {
  log('Request GET /topic/%s/votes', req.params.id);

  api.topic.getVotes(req.params.id, function (err, votes) {
    if (err) return _handleError(err, req, res);
