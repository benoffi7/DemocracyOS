...
class TopicFilter {
  ...
  set (items, search) {
    ...
    bus.emit('topic-filter:update', filteredItems, this.getFilter(), search);
  ...
  filter (items) {
    return items.filter(item => {
      ...
      //Filter del Search
      if(! this.filterSearch(item)) return false;
  ...
  filterSearch(item){
	if(this._filter.search != '' 
		&& item.mediaTitle.toLowerCase().indexOf(this._filter.search.toLowerCase()) === -1
		&& item.author.fullName.toLowerCase().indexOf(this._filter.search.toLowerCase()) === -1
		&& item.tag.name.toLowerCase().indexOf(this._filter.search.toLowerCase()) === -1) return false;
	return true;
  }
