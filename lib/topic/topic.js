...
import { findTopics, findTopicProposal } from '../topic-middlewares/topic-middlewares';
import { findForum } from '../forum-middlewares/forum-middlewares';
import { clearUsersTypesStore, findUserTypeProposal } from '../usertype-middlewares/usertype-middlewares';
import sidebar from '../sidebar/sidebar';
import user from '../user/user';
import Article from '../proposal-article/proposal-article';
import Options from '../proposal-options/proposal-options';
import Modal from '../proposal-modal/proposal-modal';
...
export function show (topic) {
  ...
  //Siempre borro lo que estaba
  o('#content-modal').empty()
  const modalContent = o('#content-modal');
  //Agrego el modal si el usuario tiene configurado el mismo
  if(user.id && user.notifications['share']){
    let modal = new Modal(topic);
    modal.appendTo(modalContent);
  }
...
page(forumRouter('/topic/:hash'), user.optional, findForum, findTopics, findTopicProposal, findUserTypeProposal, (ctx, next) => {
  log(`rendering Topic ${ctx.params.hash}`);

  if (!ctx.topic) {
    log('Topic %s not found', ctx.params.hash);
    return next();
  }

  show(ctx.topic, ctx.userLogged);

  title(ctx.topic.mediaTitle);

  log('render %s', ctx.params.hash);
});
