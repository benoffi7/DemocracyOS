...
app.get('/user/profile/:username', function (req, res) {
  log('Request user/%s', req.params.username);

  api.user.getProfile(req.params.username, function (err, user) {
    if (err) return _handleError(err, req, res);
  ...
  api.user.get(req.params.id, function (err, user) {
    ...
    var keys = [
	  'id firstName lastName username level usertype locale email emailValidated profiles createdAt updatedAt',
           'web facebook twitter linkedin googlePlus youtube profilePictureUrl disabledAt notifications favorites'
    ].join(' ');
    res.json(expose(keys)(user));
  });
});
...
app.post('/user/:id', restrict, staff, function (req, res) {
  ...
  var user= new User({firstName: req.body.firstName, lastName: req.body.lastName});
  user.username= normalize(user.fullName);  
  
  User.findOne({username: user.username}, function(err, userExists){
    if (err) return _handleError(err, req, res);
    
    if(userExists && userExists._id != req.params.id){
        return _handleError("El username ya existe", req, res);
    }else{
...
app.post('/user/:id/favorite/:topicId', restrict, function (req, res) {
  log('Request POST add favorite', req.params.id);

  api.user.get(req.params.id, function (err, user) {
    if (err) return _handleError(err, req, res);

    user.addFavorite(req.params.topicId);
    user.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
      log('add favorite topic to user %s', user.id);
      res.json(200);
    });
  });
});

app.post('/user/:id/unfavorite/:topicId', restrict, function (req, res) {
  log('Request POST add favorite', req.params.id);

  api.user.get(req.params.id, function (err, user) {
    if (err) return _handleError(err, req, res);

    user.deleteFavorite(req.params.topicId);
    user.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
      log('add favorite topic to user %s', user.id);
      res.json(200);
