...
class UserlistStore extends Store {
   ...
   * @api public
   */
  findUserNameProfile (username) {
    if (this.item.get(username)) return Promise.resolve(this.item.get(username));
...
//  }
  ...
  addFavorite (id, topicId) {
    ...
    if (!item) {
      return this.findOne(id).then(() => {
        return this.addFavorite(id, topicId);
      });
    }
    let addFavorite = new Promise((resolve, reject) => {        
      request
        .post(`${this.url(id)}/favorite/${topicId}`)
        .end((err, res) => {
          if (err || !res.ok) return reject(err);
  ...
  deleteFavorite (id, topicId) {
    let item = this.item.get(id);
    if (!item) {
      return this.findOne(id).then(() => {
        return this.deleteFavorite(id, topicId);
      });
    }
    let deleteFavorite = new Promise((resolve, reject) => {        
      request
        .post(`${this.url(id)}/unfavorite/${topicId}`)
        .end((err, res) => {
          if (err || !res.ok) return reject(err);
