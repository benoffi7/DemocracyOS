...
app.get('/userstypes/all', function (req, res) {
  ...
  api.usertype.all(function (err, userstypes) {
    ...
    res.json(userstypes.map(expose('id hash name createdAt deletedAt')));
  });
});
...
app.get('/userstypes/allactive', function (req, res) {
  ...
  api.usertype.allActive(function (err, userstypes) {
    ...
    res.json(userstypes.map(expose('id hash name projectsTypes createdAt deletedAt')));
  });
});
...
app.get('/userstypes/:id', function (req, res) {
  ...
  api.usertype.get(req.params.id, function (err, userstypesDoc) {
    ...
    var keys = [
      'id hash name projectsTypes createdAt deletedAt'
    ].join(' ');
...
app.post('/userstypes/create', restrict, staff, function (req, res, next) {
  ...
  api.usertype.create(req.body, function (err, userstypesDoc) {
    ...
    var keys = [
      'id hash name  createdAt deletedAt'
    ].join(' ');
...
app.post('/userstypes/:id/project/:projectcId', restrict, function (req, res) {
  log('Request POST add project', req.params.id);

  api.usertype.get(req.params.id, function (err, usertype) {
    if (err) return _handleError(err, req, res);

    usertype.addProject(req.params.projectcId);
    usertype.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
      log('add project to usertype %s', usertype.id);
      res.json(200);
    });
  });
});

app.post('/userstypes/:id/unproject/:projectcId', restrict, function (req, res) {
  log('Request POST delete project', req.params.id);

  api.usertype.get(req.params.id, function (err, usertype) {
    if (err) return _handleError(err, req, res);

    usertype.deleteProject(req.params.projectcId);
    usertype.save(function (err, saved) {
      if (err) return _handleError(err, req, res);
      log('delete project to usertype %s', usertype.id);
      res.json(200);
    });
  });
});
